...
func TestCreateRequest(t *testing.T) {
	key := url.QueryEscape("asdf") + "&" + url.QueryEscape("")
	p := &Provider{
		Secret:      "asdf",
...
func TestSign(t *testing.T) {
	...
	signed, err := SignLTIRequest(vals,
		"http://www.imsglobal.org/developers/LTI/test/v1p1/tool.php",
		"post", []byte(key))
...
func TestBaseString(t *testing.T) {
	...
	var formParameters []KV

	allParameters := mergeRequestParameters(queryParameters, oauthParameters, formParameters)

	baseUrlString, err := GetOAuthBaseString("GET", "http://photos.example.net:8001/Photos", allParameters)
	if err != nil {
		panic(err)
	}
