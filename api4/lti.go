...
func signupWithLTI(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if err := json.Unmarshal([]byte(string(data)), &ltiLaunchData); err != nil {
	...
	if !utils.ValidateLTIRequest(c.GetSiteURLHeader()+"/login/lti", c.App.Config().LTISettings.GetKnownLMSs(), ltiLaunchDataRequest) {
		c.Err = model.NewAppError("loginWithLti", "api.lti.signup.app_error", map[string]interface{}{"ErrorCode": "5"}, "", http.StatusBadRequest)
		return
	}

	props := model.MapFromJson(r.Body)
	fmt.Println(props)
	fmt.Println(props["password"])
